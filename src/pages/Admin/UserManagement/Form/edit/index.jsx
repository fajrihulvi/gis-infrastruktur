...
const AdminFormEditUserManagement = () => {
  ...
  const methods = useForm({
    defaultValues: {
      ...
      dinas: location.state.dinas_id ?? '',
      nip: location.state.nip ?? ''
    },
  });
  ...
  const { mutate } = useMutation({
    mutationFn: async (someProps) => {
      try {
        const response = await updateAdmin(someProps);
        return response;
      } catch (error) {
          ...
          <Flex flexDirection={`column`}>
            <Breadcrumb p={`20px`}>
              ...
              <BreadcrumbItem isCurrentPage>
                <BreadcrumbLink href='#' color={`#138CEC`}>{location.state.name}</BreadcrumbLink>
              </BreadcrumbItem>
            </Breadcrumb>
              ...
              <Flex flexDirection={`column`}>
                <Box p={`20px`} borderBottom={`1px`} borderColor={`#D8D8D8`}>
                  <Flex alignItems={`center`}>
                    <Text fontSize='2xl'>Edit User admin</Text>
                  </Flex>
                </Box>
                <Box p={`20px`}>
                    <FormProvider {...methods}>
                      <Flex flexDirection={`column`} gap={4}>
                          ...
                          <Flex justifyContent={`flex-end`} gap={4}>
                              ...
                              <Button onClick={() => methods.handleSubmit(onSubmit)()} bg={`#3758F9`} color={`white`} >Update user</Button>
                          </Flex>
                      </Flex>
                    </FormProvider>
                </Box>
...
export default AdminFormEditUserManagement;
