...
import { createCategory } from "../../../../../services/categoryInfrastructure";
import { useMutation } from "react-query";
import { Controller, FormProvider, useForm } from "react-hook-form";
import { useNavigate } from "react-router-dom";
import { getUser } from "../../../../../app/store/auth";

const AdminFormAddInfrastructureCategory = () => {
  ...
  const { mutate } = useMutation({
    mutationFn: async (someProps) => {
      try {
        const response = await createCategory(someProps);
        return response;
      } catch (error) {
          ...
          <Flex flexDirection={`column`}>
            <Breadcrumb p={`20px`}>
              ...
              <BreadcrumbItem isCurrentPage>
                <BreadcrumbLink href='#' color={`#138CEC`}>Tambah Kategori Infrastruktur</BreadcrumbLink>
              </BreadcrumbItem>
            </Breadcrumb>
              ...
              <Flex flexDirection={`column`}>
                <Box p={`20px`} borderBottom={`1px`} borderColor={`#D8D8D8`}>
                  <Flex alignItems={`center`}>
                    <Text fontSize='2xl'>Tambah Kategori Infrastruktur</Text>
                  </Flex>
                </Box>
                <Box p={`20px`}>
...
export default AdminFormAddInfrastructureCategory;
